...
<?php 
    ...
    $stmt = $connect->prepare("
        SELECT a.*, r.recipe_name 
        FROM allergies a 
        JOIN recipes r ON a.allergy_recipe_id = r.recipe_id 
        WHERE a.allergy_name LIKE ? OR r.recipe_name LIKE ? 
        LIMIT ?, ?
    ");
...
<div class="admin-box-box">
    <div class="top">
        ...
        <a href="./allergy_add.php"><button type="button" class="btn_add btn btn-primary">Add Allergy</button></a>
        <div class="search">
            <div class="search-container-box">
                <form method="GET" action="allergy_management.php">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td><a href="./allergy_edit.php?id=<?=$row['allergy_id']?>"><button type="button" id="btn-action">Edit</button></a></td>
                            <td><button id="btn-action" onclick="confirmDelete(<?=$row['allergy_id']?>)">Delete</button></td>
                        </tr>
                    <?php $count++; } 
                } else { ?>
                    <tr>
                        <td colspan="7">No recipes or allergies found</td>
                    </tr>
                <?php } ?>
        </table>
    </div>
    <div class="pagination">
        ...
        // Pagination buttons
        if ($page > 1) {
            echo "<a href='./allergy_management.php?page=" . ($page - 1) . "&query=" . urlencode($search_query) . "' class='btn_page'>Previous</a>";
        }

        for ($i = 1; $i <= $total_page; $i++) {
            echo "<a href='./allergy_management.php?page=" . $i . "&query=" . urlencode($search_query) . "' class='btn_page'>$i</a>";
        }

        if ($page < $total_page) {
            echo "<a href='./allergy_management.php?page=" . ($page + 1) . "&query=" . urlencode($search_query) . "' class='btn_page'>Next</a>";
        }
        ?>
    </div>
</div>
...
<script>
    function confirmDelete(allergyId) {
    if (confirm("Are you sure you want to delete this allergy?")) {
        window.location.href = './delete_allergy.php?id=' + allergyId;
    }
}
</script>
</html>
